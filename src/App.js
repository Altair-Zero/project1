...
export const App = () => {
  ...
  return (
    <Box display="grid" gridTemplateColumns="1fr 1800px 1fr" sx={{background: "#fdfdfd"}}>
      <Box display="flex" flexDirection="column" gridArea="1 / 2 / 2 / 3" sx={{background: "white", minHeight: "100vh", width: "1800px"}}>
        <Box width="100%" textAlign="center" mt={2} sx={{fontWeight: "800", fontSize: "25px"}}>
            ...
            <Box height="calc(100vh - 86px)" sx={{overflowY: "auto"}}>
              ...
              {(users && posts) && users.map((it, ind) => (
                ...
                >
                  ...
                  <Box display="flex"><Box sx={{fontWeight: "600"}}>Никнейм:&nbsp;</Box>{it.username}</Box>
